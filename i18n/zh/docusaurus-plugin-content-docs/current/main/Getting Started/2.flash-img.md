...
2. Debian,Ubuntu 系统是使用RK官方提供SDK适配编译的系统，对于有 ISP、GPU、编解码、NPU需求的用户更加合适。使用RKDevTool烧录到eMMC，或者使用SDDiskTool烧录到SD卡。
3. Armbian，Openwrt是ArmSoM为开源爱好者提供的更多选择。
:::

## 2.2 烧录Ubuntu，Debian, 安卓系统
...
```
请参照产品相关说明进行操作，不同产品进入 Maskrom或者Recovery 模式的方式基本相同。
...
使用一根Type-C线一端连接到产品的OTG接口，另一端连接电脑的usb接口（切勿连接电脑转接HUB），按住板载的Maskrom按键之后供电，三秒之后松开按键，正常情况下会进入 Maskrom 状态
