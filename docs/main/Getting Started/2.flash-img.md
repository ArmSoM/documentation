...
2. The Debian/Ubuntu system is a system compiled using the SDK officially provided by RK. It is more suitable for users with ISP, GPU, codec, and NPU requirements. Use RKDevTool to burn to eMMC, or use SDDiskTool to burn to SD card.
3. Armbian, Openwrt are more choices provided by ArmSoM for open source enthusiasts.
:::


## 2.2 Burn Ubuntu, Debian, Android system
...
```
Please refer to the product-related instructions for operation. The method to enter Maskrom mode is largely similar across different products.

1. Prepare a Type-C cable for image burning.  
2. Disconnect all cables that may supply power to the product, such as USB cables.  
3. Connect one end of a Type-C cable to the product’s OTG interface and the other end to the computer’s USB port (avoid connecting through a computer adapter hub), then open the RKDevTool software.  
4. Press and hold the MASKROM or Recovery button, then power the product using a DC power supply.  
5. Wait for the software to indicate that a MASKROM/LOADER device has been detected (as shown in the figure below), then release the button.  
...
## 2.3 Burn Armbian, Openwrt system
...
Connect one end of a Type-C cable to the OTG port of the product and the other end to the USB port of the computer (do not connect through a computer adapter hub).  Press and hold the onboard Maskrom button while powering on the device, then release the button after three seconds. Under normal circumstances, the device will enter Maskrom mode.
